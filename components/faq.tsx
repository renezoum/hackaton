      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-12 relative">
          <input
            ...
            onChange={(e) => {
              ...
              // Auto-open first matching item when searching
              if (e.target.value.trim()) {
                const firstMatch = filteredCategories[0]?.items[0]?.id
                if (firstMatch && !openItems.includes(firstMatch)) {
                  setOpenItems([firstMatch])
                }
              }
            }}
        ...
        {searchQuery && filteredCategories.length === 0 && (
          <div className="text-center py-12 space-y-4">
            ...
            <p className="text-gray-500">Essayez avec d'autres mots-clés ou contactez EVA pour de l'aide</p>
          </div>
        )}
        ...
        {filteredCategories.length > 0 && (
          ...
          {filteredCategories.map((category) => (
            <div key={category.id} className={`${category.bgColor} rounded-3xl p-3 md:p-8 border-l-4 ${category.borderColor} space-y-4`}>
              {/* Category Header */}
                ...
                {category.items.map((item, idx) => (
                  <div
                    ...
                    className={`bg-white rounded-2xl border-2 border-gray-200 overflow-hidden hover:border-green-300 hover:shadow-md transition-all duration-300 ${
                      idx === category.items.length - 1 ? 'border-green-200 border-l-4' : ''
                    }`}
                  >
                    <button
                    ...
                    >
                      ...
                      <span className={`text-green-600 text-2xl transform transition-transform duration-300 flex-shrink-0 ml-4 ${openItems.includes(item.id) ? 'rotate-180' : ''}`}>
                        ▼
                      </span>
                    </button>
